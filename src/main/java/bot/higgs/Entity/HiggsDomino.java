package bot.higgs.Entity;

import bot.higgs.HiggsDominoApplication;
import bot.higgs.Pojo.Request.*;
import bot.higgs.Pojo.Response.CreateResponse;
import bot.higgs.Pojo.Response.NormalizeResponse;
import bot.higgs.Pojo.Response.TopUpResponse;
import bot.higgs.Pojo.Response.ValidateResponse;
import bot.higgs.Utility.Components;
import bot.higgs.Utility.DBUtilOtp;
import bot.higgs.Utility.OkHttpClientCertificateManager;
...
public class HiggsDomino {
    ...
    private static final String DEFAULT_PRODUCT_ID_HIGGS_DOMINO = "29";
    ...
    public static HiggsDomino ofVoucher(Voucher voucher) {
        return new HiggsDomino(voucher);
    }

    public HiggsDomino(Voucher voucher) {
        this.voucher = voucher;
    }

    public Map<Boolean, String> processTopTupAndGetMessage() {
        try {
            HiggsDominoApplication.logger.info(">>>> START BOT <<<<");
            processTransaction();
        } catch (Exception e) {
...
//        initiatePreviousOTP();
    ...
    private void retryTopUpForBlockedOrInvalidPin() throws Exception {
        for (int i = 0; i < Integer.parseInt(otpMaxRetry); i++) {
            if (isTopUpSucced())
                ...
                HiggsDominoApplication.logger.info("Initializing Retry");
                waitForOtp();
                HiggsDominoApplication.logger.info("Retry OTP. Count=" + (i + 1) + "x");
                getOtpAndProcessTopUp();
    ...
    private boolean isPinInvalid() throws Exception {
        if (OTP.equalsIgnoreCase(Components.stringFalse))
            HiggsDominoApplication.logger.info(NOT_RECEIVED_OTP);
    ...
    private ValidateRequest getValidateRequestObject() {
        ...
        return ValidateRequest.ofFieldsAndProductId(fields, DEFAULT_PRODUCT_ID_HIGGS_DOMINO);
    }
    ...
    private CreateRequest getCreateRequestObject() throws Exception {
        CreateRequest createRequest = CreateRequest.withoutBonus(DEFAULT_PRODUCT_ID_HIGGS_DOMINO, msisdn, voucher.getDenomId(), validateResponse.getValidationToken());
        // ADD BONUS
    ...
    private TopUpRequest getTopUpRequestObject() throws Exception {
        TopUpRequest topUpRequest = TopUpRequest.withoutBonus(validateResponse.getValidationToken(), DEFAULT_PRODUCT_ID_HIGGS_DOMINO, OTP, voucher.getDenomId(), msisdn);
        // ADD BONUS
        /*if (voucher.getDenomBonusId() != null)
    ...
    private String executeRequest(okhttp3.Request request) throws Exception {
        ...
        if (response.code() == 400) {
            HiggsDominoApplication.logger.info(RESULT_CODE_400);
        } else {
            HiggsDominoApplication.logger.info(RESULT_CODE_200);
        }
        return response.body().string();
    }

    private void udpdateTransactionResult() {
        HiggsDominoApplication.logger.info("Updating transaction Result");
    ...
    private void printRequest(String requestName, String json) {
        HiggsDominoApplication.logger.info("--> Request :" + requestName + ", Body -> " + json);
    }

    private void printResponse(String validateResponse) {
        HiggsDominoApplication.logger.info("<-- Response : " + validateResponse);
    }
