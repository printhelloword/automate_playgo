...
public class CodMobile {
    ...
    private static final String DEFAULT_PRODUCT_ID_COD_MOBILE = "9";
    ...
    public static CodMobile ofVoucher(Voucher voucher) {
        return new CodMobile(voucher);
    }

    public CodMobile(Voucher voucher) {
        this.voucher = voucher;
    }

    public Map<Boolean, String> processTopTupAndGetMessage() {
        try {
            CodMobileApplication.logger.info(">>>> START BOT <<<<");
            processTransaction();
        } catch (Exception e) {
...
//        initiatePreviousOTP();
    ...
    private void retryTopUpForBlockedOrInvalidPin() throws Exception {
        for (int i = 0; i < Integer.parseInt(otpMaxRetry); i++) {
            if (isTopUpSucced())
                ...
                CodMobileApplication.logger.info("Initializing Retry");
                waitForOtp();
                CodMobileApplication.logger.info("Retry OTP. Count=" + (i + 1) + "x");
                getOtpAndProcessTopUp();
    ...
    private boolean isPinInvalid() throws Exception {
        if (OTP.equalsIgnoreCase(Components.stringFalse))
            CodMobileApplication.logger.info(NOT_RECEIVED_OTP);
    ...
    private ValidateRequest getValidateRequestObject() {
        ...
        return ValidateRequest.ofFieldsAndProductId(fields, DEFAULT_PRODUCT_ID_COD_MOBILE);
    }
    ...
    private CreateRequest getCreateRequestObject() throws Exception {
        CreateRequest createRequest = CreateRequest.withoutBonus(DEFAULT_PRODUCT_ID_COD_MOBILE, msisdn, voucher.getDenomId(), validateResponse.getValidationToken());
        // ADD BONUS
    ...
    private TopUpRequest getTopUpRequestObject() throws Exception {
        TopUpRequest topUpRequest = TopUpRequest.withoutBonus(validateResponse.getValidationToken(), DEFAULT_PRODUCT_ID_COD_MOBILE, OTP, voucher.getDenomId(), msisdn);
        // ADD BONUS
        /*if (voucher.getDenomBonusId() != null)
    ...
    private String executeRequest(okhttp3.Request request) throws Exception {
        ...
        if (response.code() == 400) {
            CodMobileApplication.logger.info(RESULT_CODE_400);
        } else {
            CodMobileApplication.logger.info(RESULT_CODE_200);
        }
        return response.body().string();
    }

    private void udpdateTransactionResult() {
        CodMobileApplication.logger.info("Updating transaction Result");
    ...
    private void printRequest(String requestName, String json) {
        CodMobileApplication.logger.info("--> Request :" + requestName + ", Body -> " + json);
    }

    private void printResponse(String validateResponse) {
        CodMobileApplication.logger.info("<-- Response : " + validateResponse);
    }
