...
public class Aov {
    ...
    private static final String DEFAULT_PRODUCT_ID_AOV = "12";
    ...
    public static Aov ofVoucher(Voucher voucher) {
        return new Aov(voucher);
    }

    public Aov(Voucher voucher) {
        this.voucher = voucher;
    }

    public Map<Boolean, String> processTopTupAndGetMessage() {
        try {
            AovApplication.logger.info(">>>> START BOT <<<<");
            processTransaction();
        } catch (Exception e) {
...
//        initiatePreviousOTP();
    ...
    private void retryTopUpForBlockedOrInvalidPin() throws Exception {
        for (int i = 0; i < Integer.parseInt(otpMaxRetry); i++) {
            if (isTopUpSucced())
                ...
                AovApplication.logger.info("Initializing Retry");
                waitForOtp();
                AovApplication.logger.info("Retry OTP. Count=" + (i + 1) + "x");
                getOtpAndProcessTopUp();
    ...
    private boolean isPinInvalid() throws Exception {
        if (OTP.equalsIgnoreCase(Components.stringFalse))
            AovApplication.logger.info(NOT_RECEIVED_OTP);
    ...
    private ValidateRequest getValidateRequestObject() {
        ...
        return ValidateRequest.ofFieldsAndProductId(fields, DEFAULT_PRODUCT_ID_AOV);
    }
    ...
    private CreateRequest getCreateRequestObject() throws Exception {
        CreateRequest createRequest = CreateRequest.withoutBonus(DEFAULT_PRODUCT_ID_AOV, msisdn, voucher.getDenomId(), validateResponse.getValidationToken());
        // ADD BONUS
    ...
    private TopUpRequest getTopUpRequestObject() throws Exception {
        TopUpRequest topUpRequest = TopUpRequest.withoutBonus(validateResponse.getValidationToken(), DEFAULT_PRODUCT_ID_AOV, OTP, voucher.getDenomId(), msisdn);
        // ADD BONUS
        /*if (voucher.getDenomBonusId() != null)
    ...
    private String executeRequest(okhttp3.Request request) throws Exception {
        ...
        if (response.code() == 400) {
            AovApplication.logger.info(RESULT_CODE_400);
        } else {
            AovApplication.logger.info(RESULT_CODE_200);
        }
        return response.body().string();
    }

    private void udpdateTransactionResult() {
        AovApplication.logger.info("Updating transaction Result");
    ...
    private void printRequest(String requestName, String json) {
        AovApplication.logger.info("--> Request :" + requestName + ", Body -> " + json);
    }

    private void printResponse(String validateResponse) {
        AovApplication.logger.info("<-- Response : " + validateResponse);
    }
