...
public class TransactionController {
    ...
    private void checkInboxisSavedAndStartTransaction() {
        if (isSaveToInboxSucceed()) {
            ...
            HiggsDomino higgsDomino = HiggsDomino.ofVoucher(voucher);

            Map<Boolean, String> transactionResult = higgsDomino.processTopTupAndGetMessage();
    ...
    private void printResponseJsonAndSaveToOutbox(JSONObject jsonObject) {
        HiggsDominoApplication.logger.info("Returning JSON : ");
        HiggsDominoApplication.logger.info(jsonObject.toString(4));
        if (inboxId != null)
    ...
    private void saveToOutbox(Outbox newBoutbox) {
        try {
            ...
            HiggsDominoApplication.logger.info("Failed Save To Outbox");
        }
    }
    ...
    private void saveToInbox(Inbox newInbox) {
        try {
            ...
            HiggsDominoApplication.logger.info(e.getMessage());
            HiggsDominoApplication.logger.info("Failed Save To DB");
        }
    }
