...
public class TransactionController {
    ...
    private void checkInboxisSavedAndStartTransaction() {
        if (isSaveToInboxSucceed()) {
            ...
            Aov aov = Aov.ofVoucher(voucher);

            Map<Boolean, String> transactionResult = aov.processTopTupAndGetMessage();
    ...
    private void printResponseJsonAndSaveToOutbox(JSONObject jsonObject) {
        AovApplication.logger.info("Returning JSON : ");
        AovApplication.logger.info(jsonObject.toString(4));
        if (inboxId != null)
    ...
    private void saveToOutbox(Outbox newBoutbox) {
        try {
            ...
            AovApplication.logger.info("Failed Save To Outbox");
        }
    }
    ...
    private void saveToInbox(Inbox newInbox) {
        try {
            ...
            AovApplication.logger.info(e.getMessage());
            AovApplication.logger.info("Failed Save To DB");
        }
    }
